...
void gemmini_t::mx_loop_ws_spad(reg_t rs1, reg_t rs2) {
  ...
  auto deposit_to_spad = [&](int M_out, int N_out) {
    ...
    // indices (0.5 byte/elem), per the "always 4-bit LUT index" requant-output convention.
    const bool out_e4m3_single = (gemmini_state.mx_out_fmt == 0) &&
                                 !gemmini_state.mx_fp8_altfmt && !gemmini_state.mx_lut_en;
    const size_t W = (gemmini_state.mx_out_fmt == 3) ? out_elems        // BF16: 1 u16/elem
                   : out_e4m3_single                 ? out_elems / 2     // E4M3-single: 1 byte/elem
                   :                                    out_elems / 4;   // 4-bit LUT / FP4: 0.5 byte/elem
